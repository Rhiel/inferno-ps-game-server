...
public class BanditCampBartenderDialogue extends DialoguePlugin {
	...
	boolean desertTreasure = player.getQuestRepository().isStarted(DesertTreasure.NAME);
	switch (stage) {
	case -1:
	    end();
	    break;
	case 0:
	    if (desertTreasure) {
		options("Ask about Desert Treasure", "Buy a drink");
		stage = 1;
	    } else {
	...
	case 6:
	    switch (buttonId) {
	    case 1:
		if (player.getInventory().contains(995, 650)) {
		    player.getInventory().remove(new Item(995, 650));
		    player.getInventory().add(new Item(3803, 1));
		    if (desertTreasure) {
			player.setAttribute("dt-beer_bought", Boolean.TRUE);
		    }
