...
public final class DialogueInterpreter {
	...
	for (int i = 0; i < messages.length; i++) {
	    player.getPacketDispatch().sendString(messages[i], interfaceId, i);
	}
	...
	player.getPacketDispatch().sendItemOnInterface(-1, 1, 131, 0);
	return player.getInterfaceManager().getChatbox();
    }
	...
	player.getPacketDispatch().sendAnimationInterface(expression, interfaceId, 0);
	if (npc) {
	    player.getPacketDispatch().sendNpcOnInterface(npcId, interfaceId, 0);
	    player.getPacketDispatch().sendString(NPCDefinition.forId(npcId).getName(), interfaceId, 1);
	} else {
	    player.getPacketDispatch().sendPlayerOnInterface(interfaceId, 0);
	    player.getPacketDispatch().sendString(player.getUsername(), interfaceId, 1);
	}
	for (int i = 0; i < messages.length; i++) {
	    player.getPacketDispatch().sendString(messages[i].toString().replace("@name", player.getUsername()), interfaceId, (i + 2));
	}
	player.getInterfaceManager().openChatbox(interfaceId);
	if (player.getAttributes().containsKey("tut-island") || TutorialSession.getExtension(player).getStage() <= TutorialSession.MAX_STAGE) {
	    Component.setUnclosable(player, player.getInterfaceManager().getChatbox());
	...
	if (title != null) {
	    player.getPacketDispatch().sendString(title.toString(), interfaceId, 0);
	}
	for (int i = 0; i < options.length; i++) {
	    player.getPacketDispatch().sendString(options[i].toString(), interfaceId, i + 1);
	}
	if (player.getAttributes().containsKey("tut-island")) {
	    Component.setUnclosable(player, player.getInterfaceManager().getChatbox());
	}
