...
public final class DuelSession extends ComponentPlugin {
	...
	case 2:
		...
		session.player.getPacketDispatch().sendString(tokens[i], interfaceId, staked ? (41 + i) : (16 + i));
		session.other.getPacketDispatch().sendString(tokens[i], interfaceId, staked ? (41 + i) : (16 + i));
	    }
	    tokens = during.toString().split("<br>");
	    for (int i = 0; i < tokens.length; i++) {
		session.player.getPacketDispatch().sendString(tokens[i], interfaceId, staked ? (41 + i) : (22 + i));
		session.other.getPacketDispatch().sendString(tokens[i], interfaceId, staked ? (41 + i) : (22 + i));
	    }
	    if (staked) {
