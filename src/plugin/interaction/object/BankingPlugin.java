...
public final class BankingPlugin extends OptionHandler {
	...
	@Override
	public Plugin<Object> newInstance(Object arg) throws Throwable {
	    ComponentDefinition.put(12, this);
	    ComponentDefinition.put(15, this);
	    return this;
	}

	@Override
	public boolean handle(final Player p, Component component, int opcode, int button, final int slot, int itemId) {
	    final Item item = component.getId() == 12 ? p.getBank().get(slot) : p.getInventory().get(slot);
		...
		case 180:
		    ...
		    case 93:
		    case 92:
			p.getBank().setNoteItems(button == 92);
			return true;
		    }
		    break;
		case 133:
		...
		case 243:
			...
			GameWorld.submit(new Pulse(1, p) {
				...
				p.getBank().takeItem(slot, p.getBank().isNoteItems() ? p.getBank().getAmount(item) : p.getInventory().getMaximumAdd(item));
				return true;
			    }
			});
			break;
		...
		case 145:
			...
			p.setAttribute("runscript", new RunScript() {
				...
				p.getBank().takeItem(slot, ammount);
				return false;
			    }
			});
