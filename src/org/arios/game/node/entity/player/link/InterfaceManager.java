...
public final class InterfaceManager {
    ...
    public Component open(Component component) {
        ...
        if (component.getDefinition() == null) {
            PacketRepository.send(Interface.class, new InterfaceContext(player, getWindowsPane(), MAIN_SLOT, MAIN_SLOT_FULL, component.getId(), false));
            return opened = component;
        }
        //		if (component.getDefinition().getContext() == null) {
        component.getDefinition().setContext(new InterfaceContext(null, getWindowsPane(), MAIN_SLOT, MAIN_SLOT_FULL, component.getId(), false));
        //		}
    ...
    public Component openSingleTab(Component component) {
        if (component.getDefinition().getContext() == null || (component.getDefinition().getContext().getComponentId() != COMPLETE_SIDEBAR_SLOT && component.getDefinition().getContext().getComponentId() != COMPLETE_SIDEBAR_SLOT_FULL)) {
            component.getDefinition().setContext(new InterfaceContext(null, getWindowsPane(), COMPLETE_SIDEBAR_SLOT, COMPLETE_SIDEBAR_SLOT_FULL,  component.getId(), false));
        }
        component.open(player);
...
/*		for (int i = 0; i < tabs.length; i++) {
    ...
    public void openDefaultTabs() {
        ...
        tabs[8] = new Component(429);
        tabs[9] = new Component(432);
    ...
    public void openTab(int slot, Component component) {
        ...
        if (component.getDefinition().getContext() == null) {
            component.getDefinition().setContext(new InterfaceContext(null, player.getInterfaceManager().getWindowsPane(), 65 + slot, 8 + slot, component.getId(), false));
        }
        component.open(player);
        tabs[slot] = component;
    }
   ...
   /* public void openChatbox(Component component) {
    ...
    public void switchWindowMode(int windowMode) {
        if (windowMode != player.getDetails().getClientInfo().getWindowMode()) {
            ...
            openWindowsPane(new Component(isResizable() ? 161 : 548));
    ...
    public void openOverlay(Component component) {
        ...
        overlay.getDefinition().setContext(new InterfaceContext(null, player.getInterfaceManager().getWindowsPane(), OVERLAY_SLOT, OVERLAY_SLOT_FULL, component.getId(), true));
        //		}
        overlay.open(player);
    }
    ...
    public void closeOverlay() {
        if (overlay != null && overlay.close(player)) {
            if (overlay.getDefinition().getContext() != null) {
                ...
                PacketRepository.send(CloseInterface.class, new InterfaceContext(player, getWindowsPane(), OVERLAY_SLOT, OVERLAY_SLOT_FULL, overlay.getId(), true));
            }
            overlay = null;
        }
    }
