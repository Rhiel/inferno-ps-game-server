...
public class PrayerAltarPlugin extends OptionHandler {
	 ...
	 */
	public enum Altar {
		ANCIENT(6552, SpellBook.ANCIENT.getConfig(), "You feel a strange wisdom fill your mind...", "You feel a strange drain upon your memory...") {
			@Override
		...
		LUNAR(17010, SpellBook.LUNAR.getConfig(), "Lunar spells activated!", "Modern spells activated!") {
			@Override
			public void pray(Player player) {
				if (!isPrayerType(player)) {
					switchToBook(player);
		...
		public void revert(Player player) {
			...
			player.getConfigManager().set(439, SpellBook.MODERN.getConfig());
		}
		...
		public void switchToBook(Player player) {
			player.getSpellBookManager().setSpellBook(SpellBook.forConfig(book));
			player.getConfigManager().set(439, book);
		}
