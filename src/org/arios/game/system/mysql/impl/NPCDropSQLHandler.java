...
public class NPCDropSQLHandler extends SQLEntryHandler<Object> {
	...
	for (String d : datas) {
	    ...
	    table.add(new ChanceItem(Integer.valueOf(tokens[0]), Integer.valueOf(tokens[1]), Integer.valueOf(tokens[2]), 1000, Double.parseDouble(tokens[3]), DropFrequency.valueOf(tokens[4]), tokens.length == 6 ? Integer.valueOf(tokens[5]) : -1));
	}
	return table;
    }
