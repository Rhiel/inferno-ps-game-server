...
public class ItemDefinition extends Definition<Item> {
    ...
    static {
        ...
        final EquipHandler equipHandler = new EquipHandler();
    ...
    public static ItemDefinition parseDefinition(int itemId, ByteBuffer buffer) {
        ...
        while (true) {
            ...
            if (opcode == 0) {
                ...
                int male_offset = buffer.get() & 0xFFFF;
            } else if (opcode == 24) {
                def.femaleWornModelId1 = buffer.getShort() & 0xFFFF;
            } else if (opcode == 25) {
                def.femaleWornModelId1 = buffer.getShort() & 0xFFFF;
                int female_offset = buffer.get() & 0xFFFF;
            } else if (opcode == 26) {
                ...
                int male_head_model = buffer.getShort() & 0xFFFF;
            } else if (opcode == 91) {
                ...
                int field3541 = buffer.getShort() & 0xFFFF;
            } else if (opcode == 149) {
                int field3542 = buffer.getShort() & 0xFFFF;
            } else if (opcode == 249) {
                int length = buffer.get() & 0xFF;
    ...
    public static void defineTemplates() {
        ...
        for (int i = 0; i < DEFINITIONS.size(); i++) {
            ...
            if (def != null && (def.maleWornModelId1 >= 0 || def.maleWornModelId2 >= 0)) {
                def.equipId = equipId++;
                // if (i < 3200)
                // System.out.println("Item [id=" + i + ", name=" +
                // def.getName() + "] equip id: " + (equipId - 1) + " [models="
                // + def.maleWornModelId1 + ", " + def.maleWornModelId2 + ", " +
                // def.femaleWornModelId1 + ", " + def.femaleWornModelId2 +
                // ".");
            }
