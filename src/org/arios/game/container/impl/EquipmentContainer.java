...
public final class EquipmentContainer extends Container {
    ...
    private static class EquipmentListener implements ContainerListener {
        ...
        public void update(Container c, ContainerEvent event) {
            ...
            PacketRepository.send(ContainerPacket.class, new ContainerContext(player, -1, 64208, 94, event.getItems(), false, slots));
            update(c);
        ...
        public void refresh(Container c) {
            ...
            PacketRepository.send(ContainerPacket.class, new ContainerContext(player, -1, 64208, 94, c.toArray(), 14, false));
            update(c);
            player.getProperties().updateDefenceAnimation();
        }
    ...
    public static void update(Player player) {
        if (!player.getInterfaceManager().hasMainComponent(465)) {
            return;
        }
        PacketRepository.send(ContainerPacket.class, new ContainerContext(player, 465, 103, 95, player.getEquipment(), false));  //TODO FIND A BETTER WAY TO DO THIS RYAN FFS
        PacketRepository.send(WeightUpdate.class, player.getPacketDispatch().getContext());
        int index = 0;
        int[] bonuses = player.getProperties().getBonuses();
        for (int i = 108; i < 121; i++) { //was 122 for summonming
            if (i == 118) { //Other Bonuses
                continue;
            }
            if(BONUS_NAMES[index] == "Summoning: ") { //SUMMONING CHEAPHAX TODO FIXME RYAN.
                index++;
            }
            int bonus = bonuses[index];
            String bonusValue = bonus > -1 ? ("+" + bonus) : Integer.toString(bonus);
            player.getPacketDispatch().sendString(BONUS_NAMES[index++] + bonusValue, 465, i);
        }
    }
}