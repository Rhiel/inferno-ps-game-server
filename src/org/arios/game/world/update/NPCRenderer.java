...
public final class NPCRenderer {
	...
	IoBuffer buffer = new IoBuffer(87, PacketHeader.SHORT);
	...
	for (NPC npc : RegionManager.getLocalNpcs(player)) {
	    ...
	    buffer.putBits(15, npc.getIndex()).putBits(3, npc.getDirection().ordinal()).putBits(1, npc.getProperties().isTeleporting() ? 1 : 0);
	    ...
	    buffer.putBits(5, offsetX).putBits(5, offsetY);
