...
public final class TradeContainer extends Container {
	...
	public void update(Container c, ContainerEvent event) {
	    PacketRepository.send(ContainerPacket.class, new ContainerContext(player, -1, 2, 24, c.toArray(), c.capacity(), false));
	    if (TradeModule.getExtension(player) != null && TradeModule.getExtension(TradeModule.getExtension(player).getTarget()) != null) {
		PacketRepository.send(ContainerPacket.class, new ContainerContext(player, -1, 2, 23, TradeModule.getExtension(TradeModule.getExtension(player).getTarget()).getContainer().toArray(), 27, false));
	    }
	    checkAlert();
	}
