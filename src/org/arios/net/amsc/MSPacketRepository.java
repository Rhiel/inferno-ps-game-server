...
public final class MSPacketRepository {
    ...
    /**
     * Updates the reply to for a player
     *
     * @param buffer The buffer.
     */
    private static void handleReplyUpdate(IoBuffer buffer) {
        String target = buffer.getString();
        String replyTo = buffer.getString();
        Player p = Repository.getPlayerByDisplay(target);
        if (p != null) {
            p.setAttribute("replyTo", replyTo);
        }
    }
    ...
    private static void handleMessage(IoBuffer buffer) {
        ...
        switch (type) {
            case 1:
                ...
                player.setAttribute("replyTo", sender);
                break;
            case 2:
