...
public final class PlayerCommandPlugin extends CommandPlugin {
	...
	case "quests":
	    ...
	    return true;
	case "emotes":
	    for (int i = 0; i < player.getEmotes().getUnlocked().length; i++) {
		player.getEmotes().unlock(i);
	    }
	    return true;
	case "reply":
		...
		player.getDialogueInterpreter().sendLongInput("Enter message to send to " + replyTo);
		player.setAttribute("runscript", new RunScript() {
    		    @Override
    		    public boolean handle() {
    		        String message = (String) getValue();
    		        if (WorldCommunicator.isEnabled()) {
    			    MSPacketRepository.sendPrivateMessage(player, replyTo, message);
    		        } else {
    		            CommunicationInfo.sendMessage(player, replyTo, message);
    		        }
    		        return true;
    		    }
		});		
