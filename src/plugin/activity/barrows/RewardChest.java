...
public final class RewardChest {
	...
	for (boolean killed : player.getSavedData().getActivityData().getBarrowBrothers()) {
		...
		player.removeAttribute("barrow:looted");//Because they haven't actually looted the chest yet.
		return;
	    }
	}
	...
	for (int i = 0; i < 2 + RandomFunction.random(3); i++) {
	    ...
	    if (item.getDefinition().getName().contains("harok") 
		    || item.getDefinition().getName().contains("uthan")
		    || item.getDefinition().getName().contains("aril") 
		    || item.getDefinition().getName().contains("orag") 
		    || item.getDefinition().getName().contains("erac")
		    || item.getDefinition().getName().contains("hrim")) {
