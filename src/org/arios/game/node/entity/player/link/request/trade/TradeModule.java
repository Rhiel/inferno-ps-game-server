...
public final class TradeModule implements RequestModule {
     ...
     */
    public static final Object[] INVENTORY_PARAMS = new Object[] { "", "", "", "Examine", "Offer-X", "Offer-All", "Offer-10", "Offer-5", "Offer", -1, 0, 7, 4, 93, (336 << 16) };

    /**
     * The trade params for the run script.
     */
    public static final Object[] TRADE_PARAMS = new Object[] { "", "", "", "Examine", "Remove-X", "Remove-All", "Remove-10", "Remove-5", "Remove", -1, 0, 7, 4, 24, (335 << 16) | 33 };

    /**
     * The partener params for the run script.
     */
    public static final Object[] PARTENER_PARAMS = new Object[] { "", "", "", "Examine", "", "", "", "", "", -1, 0, 7, 4, 23, (335 << 16) | 34 };
	...
	case 0:
	    ...
	    player.getPacketDispatch().sendString("Trading With: " + target.getUsername(), 335, 17);
	    player.getPacketDispatch().sendString(target.getUsername() + " has " + (target.getInventory().freeSlots() == 0 ? "no" : (target.getInventory().freeSlots())) + " free inventory slots.", 335, 23);
	    player.getPacketDispatch().sendString(getAcceptMessage(), 335, 38);
	    break;
	case 1:
	    player.getPacketDispatch().sendString("<col=00FFFF>Trading with:<br>" + "<col=00FFFF>" + target.getUsername().substring(0, 1).toUpperCase() + target.getUsername().substring(1), 334, 44);
	...
	if (stage == 1) {
	    ...
	    if (otherModified) {
		player.getPacketDispatch().sendString("Trade modified", 335, 41);
		player.getConfigManager().set(1043, 1);
	    }
	    if (modified) {
		player.getPacketDispatch().sendString("Trade modified", 335, 40);
	...
	player.getPacketDispatch().sendAccessMask(1086, 0, 336, 0, 27);
	player.getPacketDispatch().sendRunScript(150, "IviiiIsssssssss", TRADE_PARAMS);
	player.getPacketDispatch().sendAccessMask(1086, (335 << 16) | 33, 335, 0, 27);
	player.getPacketDispatch().sendRunScript(150, "IviiiIsssssssss", PARTENER_PARAMS);
	player.getPacketDispatch().sendAccessMask(1086, (335 << 16) | 34, 335, 0, 27);
	return this;
    }
	...
	displayOffer(TradeModule.getExtension(target).getContainer(), 40);
	return this;
    }
