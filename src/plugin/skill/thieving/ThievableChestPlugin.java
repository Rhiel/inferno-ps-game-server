...
public final class ThievableChestPlugin extends OptionHandler {
	...
	ObjectBuilder.add(new GameObject(2571, new Location(3297, 3940, 0), 3));
	ObjectBuilder.add(new GameObject(2571, new Location(3283, 3946, 0), 2));
	ObjectBuilder.add(new GameObject(2571, new Location(3288, 3946, 0), 2));
	return this;
    }
	...
	case "open":
	    if (chest != null) {
		chest.open(player, (GameObject) node, chest);
		return true;
	    }
	...
	case "search for traps":
	    chest.searchTraps(player, (GameObject) node, chest);
	    return true;
	}
	return true;
    }
    ...
    /**
     * Potential rewards from the rogue's chest.
     */
    private static final ChanceItem[] ROGUES_REWARDS = new ChanceItem[] { 
   	new ChanceItem(1622, 5, 5, DropFrequency.COMMON),//uncut emerald
   	new ChanceItem(1624, 5, 5, DropFrequency.COMMON),//uncut sapphire
   	new ChanceItem(360, 15, 15, DropFrequency.COMMON),//raw tuna
   	new ChanceItem(593, 25, 25, DropFrequency.COMMON),//ashes
   	new ChanceItem(591, 3, 3, DropFrequency.COMMON),//tinderbox
   	new ChanceItem(558, 25, 25, DropFrequency.COMMON),//mind rune
   	new ChanceItem(995, 1000, 1000, DropFrequency.COMMON),//coins
   	new ChanceItem(1602, 1, 3, DropFrequency.UNCOMMON),//cut diamond
   	new ChanceItem(352, 10, 10, DropFrequency.UNCOMMON),//pike
   	new ChanceItem(454, 13, 13, DropFrequency.UNCOMMON),//coal
   	new ChanceItem(441, 10, 10, DropFrequency.UNCOMMON),//iron ore
   	new ChanceItem(562, 40, 40, DropFrequency.UNCOMMON),//chaos rune
   	new ChanceItem(560, 30, 30, DropFrequency.UNCOMMON),//death rune
   	new ChanceItem(554, 30, 30, DropFrequency.UNCOMMON),//fire rune
   	new ChanceItem(1616, 2, 2, DropFrequency.RARE),//dragonstone
   	new ChanceItem(386, 10, 10, DropFrequency.RARE),//shark
    };
	...
	ROGUE_CHEST(2571, 84, 100, new Item[] { new Item(995, 10) }, 14), TEN_COIN(2566, 13, 7.8, new Item[] { new Item(995, 10) }, 7), NATURE_RUNE(2567, 28, 25, new Item[] { new Item(995, 3), new Item(561, 1) }, 8), FIFTY_COIN(2568, 43, 125, new Item[] { new Item(995, 50) }, 55), STEEL_ARROWHEADS(2573, 47, 150, new Item[] { new Item(41, 5) }, 210), BLOOD_RUNES(2569, 59, 250, new Item[] { new Item(995, 500), new Item(565, 2) }, 135), PALADIN(2570, 72, 500, new Item[] { new Item(995, 1000), new Item(383, 1), new Item(449, 1), new Item(1623, 1) }, 120);
	 ...
	 * @param object the object.
	 */
	private void open(final Player player, final GameObject object, Chest chest) {
	    if (isRespawning(chest)) {
	 ...
	 * @param object the object.
	 */
	private void searchTraps(final Player player, final GameObject object, final Chest chest) {
	    if(object.getId() == 2571){
		return;
	    }
		...
		public boolean pulse() {
			...
			player.unlock();
			if(object.getId() == 2571){
			    final ChanceItem item = RandomFunction.getChanceItem(ROGUES_REWARDS);
			    player.lock(1);
			    player.getInventory().add(new Item(item.getId(), RandomFunction.random(item.getMinimumAmount(), item.getMaximumAmount())));
			    player.sendMessage("You find some loot inside.");
			} else {
			    for (Item i : rewards) {
	 ...
	 */
	public void setRespawn(Chest c) {
	    c.currentRespawn = GameWorld.getTicks() + (int) (respawn / 0.6);
	}
	 ...
	 * @return {@code True} if so.
	 */
	public boolean isRespawning(Chest c) {
	    return c.currentRespawn > GameWorld.getTicks();
	}
