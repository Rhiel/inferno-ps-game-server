...
public class AchievementDiary implements SavingModule {
	...
	clear(player);
	sendString(player, "<red>Achievement Diary - " + type.getName() + " <blue>- Members only<br><br>", 2);
	StringBuilder builder = new StringBuilder((isComplete() ? GREEN : hasStarted() ? YELLOW : "<red>") + type.getName() + " Area Tasks<br><br>");
	boolean complete;
	/*
	 * if (!hasStarted()) { builder.append(type.getInfo()); }
	 */
	for (int level = 0; level < type.getAchievements().length; level++) {
	    builder.append("<br><br>" + getStatus(level) + (level == 0 ? "Easy" : level == 1 ? "Medium" : "Hard") + "");
	...
	sendString(player, builder.toString(), 4);
