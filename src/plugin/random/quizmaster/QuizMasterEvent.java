...
public final class QuizMasterEvent extends AntiMacroEvent {
	...
	/*if (player.getName().equals("vexia")) {
	    return false;
	}
	...
	/*setRegion();
	if (login) {
	    quizMaster = NPC.create(2477, getLocation(32, 32));
	    quizMaster.init();
	    teleport();
	    if (startedQuiz) {
		startQuiz();
	...
	region = DynamicRegion.create(7754);
	registerRegion(region.getId());
    }

    /**
     * Teleports the player to the quiz.
     */
   /* private void teleport() {
	player.getAppearance().setAnimations(Animation.create(2378));
	player.getAppearance().sync();
	player.faceLocation(getLocation(32, 32));
	player.getProperties().setTeleportLocation(getLocation(32, 28));
	startQuiz();
    }

    /**
     * Sets the quiz master.
     */
   /* private void setQuizMaster(boolean teleport) {
	if (quizMaster == null) {
   ...
   /* private void startQuiz() {
	player.getDialogueInterpreter().open(2477, this);
	player.sendMessage("Answer four questions correctly in a row to be teleported back where you came from.");
    }
     ...
     * @return the location.
     */
   /* private Location getLocation(int xOff, int yOff) {
	return region.getBaseLocation().transform(xOff, yOff, 1);
    }*/
